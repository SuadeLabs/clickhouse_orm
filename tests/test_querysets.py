    ...
    def test_filter_with_q_objects(self):
        ...
        self._test_qs(
            qs.filter(first_name="Cassady").filter(
                Q(last_name="Knapp") | Q(last_name="Rogers") | Q(last_name="Gregory")
            ),
            2,
        )
        self._test_qs(
...
class AggregateTestCase(TestCaseWithData):
    ...
    def test_aggregate_with_wrong_grouping(self):
        with self.assertRaises(AssertionError):
            Person.objects_in(self.database).aggregate(weekday="toDayOfWeek(birthday)", count="count()").group_by(
                "first_name"
            )
    ...
    def test_aggregate_on_aggregate(self):
        with self.assertRaises(NotImplementedError):
            Person.objects_in(self.database).aggregate(weekday="toDayOfWeek(birthday)", count="count()").aggregate(
                s="sum(height)"
            )
